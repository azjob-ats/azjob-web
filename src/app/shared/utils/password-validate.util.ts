...
export class PasswordValidateUtil {
  public static validatePasswordNotHaveSevenCharacters(
    password: string
  ): Observable<boolean> {
    const PASSWORD_NOT_HAVE_SEVEN_CHARACTERS =
      password.length < MIN_CHARACTERS_PASSWORD;

    if (PASSWORD_NOT_HAVE_SEVEN_CHARACTERS) {
      return throwError(
        () =>
          'As recomendações básicas indicam que sua senha deve conter pelo menos 8 caracteres'
      );
    }
    return of(true);
  }

  public static validatePasswordIfExistOneLetter(
    password: string
  ): Observable<boolean> {
  ...
  public static validatePasswordIfExistUppercaseLetter(
    password: string
  ): Observable<boolean> {
    ...
    if (!PASSWORD_NOT_HAVE_UPPERCASE_LETTER) {
      return throwError(
        () => 'Sua senha deve conter pelo menos uma letras maiúscula'
      );
    }
    return of(true);
  }

  public static validatePasswordIfExistLowercaseLetter(
    password: string
  ): Observable<boolean> {
    const letterRegex = /[a-z]/;
    ...
    if (!PASSWORD_NOT_HAVE_LOWERCASE_LETTER) {
      return throwError(
        () => 'Sua senha deve conter pelo menos uma letras minúscula'
      );
    }
    return of(true);
  }

  public static validatePasswordIfExistOneNumber(
    password: string
  ): Observable<boolean> {
    const numberRegex = /\d/;
  ...
  public static validatePasswordIfExistOneSpecialCharacter(
    password: string
  ): Observable<boolean> {
    const specialCharRegex = /[!@#$%^&*()_+{}[\]:;<>,.?~\\/-]/;

    const PASSWORD_NOT_HAVE_ONE_SPECIAL_CARACTER =
